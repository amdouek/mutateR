...
#'
#' @param sequence_context Character vector of 34bp sequences.
...
predict_deepcpf1_python <- function(sequence_context) {
  ...
  if (nchar(sequence_context[1]) != 34) {
    warning("Seq-DeepCpf1 requires exactly 34bp sequence context. Returning NAs.")
    return(rep(NA_real_, length(sequence_context)))
  }

  encoded_data <- one_hot_encode_dna(sequence_context)

  # --- 4. Define Python Architecture (Persistent) ---

  # FIX: We use py_eval to check if the function exists in the Python global namespace
  # 'globals()' is a standard Python dictionary containing all global variables
  is_defined <- tryCatch({
...
def build_and_predict_deepcpf1(weights_path, input_data):
    ...
    try:
        ...
        if _DEEPCPF1_MODEL_CACHE is not None:
            ...
            # --- Build the Architecture (Only runs once) ---
