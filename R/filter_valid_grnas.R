...
filter_valid_grnas <- function(exon_gr,
                               genome,
                               species,
                               score_method = c("ruleset1","azimuth","deephf")) {
  ...
  if (n_exons <= 2) {
    warning("Transcript has ", n_exons, " exon(s). Skipping phase‑compatibility checks and returning scored guides only.")
    # Find and score guides without calling check_exon_phase()
    grna_sites <- find_cas9_sites(exon_gr, genome, pam = "NGG", protospacer_length = 20)
    if (is.null(grna_sites) || length(grna_sites) == 0) {
      warning("No Cas9 sites found.")
      return(NULL)
    }
  ...
  grna_sites <- find_cas9_sites(exon_gr, genome, pam = "NGG", protospacer_length = 20)
  if (is.null(grna_sites) || length(grna_sites) == 0) {
    warning("No Cas9 sites found within provided exons.")
    return(NULL)
  }
