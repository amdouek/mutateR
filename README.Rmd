...
The app's data table reports the exons in the pair, the two protospacer sequences, their on-target scores, as well as the (approximate) genomic- and transcript-specific deletion sizes. Additional metadata from the `$pairs` data.frame not displayed in the app is included in the exported CSV.

Future updates will implement automated genotyping primer design for each gRNA pair, and order sheet (e.g. IDT/Thermo)-friendly export styles.
...
-   Constitutive exon filtering between protein-coding transcripts within a gene
